...
child.on('exit',()=>{clearTimeout(timer);fs.writeFileSync(path.join(out,'log.txt'),log);try{const r=JSON.parse(fs.readFileSync(marker,'utf8'));if(!r.ready||!r.packaged||r.version!==require('../package.json').version||Object.values(r.hotkeyStatus).some(v=>v!==true))throw Error('Startup validation failed');console.log(JSON.stringify({...r,output:out}))}catch(e){console.error(e);process.exitCode=1}})
