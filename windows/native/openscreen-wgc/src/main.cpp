...
bool parseConfig(const std::string& json, CaptureConfig& config) {
    ...
    config.cropX = findInt(json, "cropX", 0);
...
int wmain(int argc, wchar_t* wideArgv[]) {
    ...
    if (std::string(argv[1]) == "--list-mics") {
    ...
    int width = config.cropW > 0 ? config.cropW : session.captureWidth();
    ...
    if (!encoder.initialize(
            ...
            session.device(),
            session.context(),
            nullptr, // Ta stores independent WAV sidecars; the screen remains silent.
            encoderOptions)) {
    ...
    if (config.webcamEnabled) {
        if (!webcamCapture.start()) {
            ...
            if (systemMixer) { systemMixer->stop(); }
        if (audioMixer) {
                audioMixer->stop();
            }
    ...
    std::thread stdinThread(readCaptureCommands, std::ref(control), [&](bool isPaused) {
        if (systemMixer) systemMixer->setPaused(isPaused);
        if (audioMixer) {
    ...
    if (systemMixer) systemMixer->beginTimeline();
    if (audioMixer) {
    ...
    micFile.close(); systemFile.close();
