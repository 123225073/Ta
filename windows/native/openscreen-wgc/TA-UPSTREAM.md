...
Ta 1.11.0 adds live source retargeting on the capture thread while keeping the encoder/audio timeline, sampled black-frame health and frame-age telemetry, same-window retry, PCM level telemetry, and default/invalid audio endpoint reconnection. `scripts/adapt-recording-resilience.cjs` replays the pinned main/WGC delta after the resize adapter. Audio endpoint reconnection converts new endpoint formats back to the original mixer format. Reference: [WASAPI loopback](https://learn.microsoft.com/en-us/windows/win32/coreaudio/loopback-recording), [device invalidation](https://learn.microsoft.com/en-us/windows/win32/coreaudio/recovering-from-an-invalid-device-error). Capture permission/exclusion imposed by another application is not bypassed.
