import {editTimeline,configureZoom,type ZoomRecipe} from './edit-actions'
...
export const cliSchema={version:1,timeUnit:'milliseconds',coordinates:'Mark coordinates are source pixels. Clip rect and zoom centers are normalized 0..1. Keyframe time is relative to clip start.',commands:['schema','list','inspect','frame','apply','undo','redo','import','export'],operations:['track.add','clip.add','clip.patch','clip.delete','clip.split','clip.speed','clip.move-many','clip.delete-many','clip.trim','zoom.configure','mark.add','mark.patch','keyframe.set'],rules:['Mutation requires revision from inspect.','apply --dry-run validates the entire batch without saving.','apply commits one validated batch; original media is never overwritten.','A recording or export in progress returns BUSY.','If an editor is open it is saved and reopened for writes.','frame uses source video time, not edited timeline time.'],example:{projectId:'ID from list',revision:0,operations:[{op:'mark.add',trackId:'mark track ID from inspect',start:1000,end:4000,mark:{tool:'arrow',x:500,y:300,width:-200,height:0,color:'#FF4D37',stroke:6,text:''}},{op:'mark.add',trackId:'mark track ID from inspect',start:1000,end:4000,mark:{tool:'text',x:150,y:250,width:300,height:60,text:'点击这里',fontSize:48,textAutoSize:true}}]}}
...
export function planEdits(base:VideoProject,operations:Record<string,unknown>[]):VideoProject {
  ...
  for(const op of operations){
    ...
    switch(op.op){
      ...
      case 'clip.delete':p.timeline=editTimeline(p.timeline,{type:'delete',ids:[needed().id]},uid);break
      case 'clip.split':p.timeline=editTimeline(p.timeline,{type:'split',ids:[needed().id],time:Number(op.time)},uid);break
      case 'clip.speed':p.timeline=editTimeline(p.timeline,{type:'speed',id:needed().id,speed:Number(op.speed)},uid);break
      case 'clip.move-many':p.timeline=editTimeline(p.timeline,{type:'move',ids:op.clipIds as string[],delta:Number(op.delta)},uid);break
      case 'clip.delete-many':p.timeline=editTimeline(p.timeline,{type:'delete',ids:op.clipIds as string[],ripple:op.ripple===true},uid);break
      case 'clip.trim': {const c=needed();if(op.side!=='left'&&op.side!=='right')throw Error('TRIM_SIDE');p.timeline=editTimeline(p.timeline,{type:'trim',id:c.id,side:op.side,time:Number(op.time),sourceDuration:p.assets?.find(a=>a.file===c.asset)?.duration??p.duration},uid);break}
      case 'zoom.configure': {const c=needed();if(pair!.t.kind!=='zoom')throw Error('ZOOM_TRACK_REQUIRED');p.timeline=replaceLinked(p.timeline,configureZoom(c,op.recipe as ZoomRecipe));break}
